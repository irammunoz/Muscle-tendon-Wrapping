...
% Stack the initial conditions to be updated by the Broyden's method
ic = [u;v;up;vp;lg];
...
a = figure(1);
a.Color = 'w';
a.Position = [1 1 768 864];

sfh1 = subplot(2,1,1);
title('\textbf{Case 2}: Broyden''s method solution','Interpreter','latex','FontSize',13);
xlim([ti(1) ti(end)]);
yyaxis left;
hold on;
plot(ti,a_lmt,'LineWidth',lw,'Color',[0, 0.4470, 0.7410]);
hh1(1) = line(ti, a_lmt, 'Marker', '.', 'MarkerSize', 20, ...
    'Color', [0, 0.4470, 0.7410]);
hold off;
ylabel(['Muscle-tendon' newline 'length (cm)'],'Interpreter','latex','FontSize',13);
yyaxis right;
hold on;
plot(ti,a_dlmt,'LineWidth',lw,'Color',[0.8500, 0.3250, 0.0980]);
hh1(2) = line(ti, a_dlmt, 'Marker', '.', 'MarkerSize', 20, ...
    'Color', [0.8500, 0.3250, 0.0980]);
hold off;
ylabel(['Muscle-tendon' newline 'velocity (cm/s)'],'Interpreter','latex','FontSize',13);
xlabel('time (s)','Interpreter','latex','FontSize',14);
grid on;
sfh1.Position = sfh1.Position + [0 0.12 0 -0.1];
...
hold off
sfh2.Position = sfh2.Position + [0 -0.05 0 0.2];

% Axes settings
xlabel('x (cm)','Interpreter','latex','FontSize',13); 
ylabel('y (cm)','Interpreter','latex','FontSize',13); 
zlabel('z (cm)','Interpreter','latex','FontSize',13);
axis equal;
view([140,35]);
grid on;
xlim([-3,3]);
...
ht = title(sprintf('Time: %0.2f sec', ti(1)));
...
vw = VideoWriter('animation_BR_NW','MPEG-4');
vw.Quality = 100;
vw.FrameRate = 60;
open(vw);
...
for i = 1:length(ti)
    ...
    set(hh1(1), 'XData', ti(i), 'YData', a_lmt(i));
    set(hh1(2), 'XData', ti(i), 'YData', a_dlmt(i));
    set(ht, 'String', sprintf('Time: %0.2f sec', ti(i)));
...
figure(100);
yyaxis left;
plot(ti,a_lmt,'LineWidth',1.5);
xlabel('time (s)','Interpreter','latex','FontSize',15);
ylabel('Muscle-tendon length (cm)','Interpreter','latex','FontSize',15);
yyaxis right;
plot(ti,a_dlmt,'LineWidth',1.5);
ylabel('Muscle-tendon velocity (cm/s)','Interpreter','latex','FontSize',15);
grid on;
...
text(-1.367519700778246,-0.940494209971831,2.284301418722947,'$^{(0)}\mbox{\boldmath $c$}_{y}=0.00$cm','Interpreter','latex','FontSize',13,'Color',color_e{1});
...
% Axes settings
xlabel('x (cm)','Interpreter','latex','FontSize',13); ylabel('y (cm)','Interpreter','latex','FontSize',13); zlabel('z (cm)','Interpreter','latex','FontSize',13);
axis equal;
view([140,35]);
grid on;
...
% Broyden algorithm constant parameters
rho = 0.9;
sigma1 = 0.001;
sigma2 = 0.001;
beta = 0.5;
...
% Initialize the Jacobian aproximation
B = jacobi(f,F,ic,Radius,p0,q0);
...
while nF(iter)>1e-10
    ...
    % Uncomment the following lines if want to use the linesearch algorithm

    et = eta(iter,nf); % compute \eta_{k}
    % If ||F(x_k + p_k)|| <= \rho ||F(x_k)|| - \sigma_{2} ||p_k||^2,
    % then let \lambda_k = 1
    if norm(f(ic+p,Radius,p0,q0)) <= (rho*nF(iter) - sigma2*norm(p)^2)
        lambda = 1;
    else 
        % Let \lambda_k be determined by Procedure 1  
        % (Approximate Norm Descent Line Search)
        [lambda, lsc_t] = linesearch(f, nF(iter), ic, p, sigma1, beta, et, Radius, p0, q0);
        lsc = lsc + lsc_t;
    end
...
%     lambda = 1;
    ...
    % Compute the approximation of the Jacobian matrix
    B = B + theta*(F*s'/(s'*s));
    iter = iter+1;  
...
up = geodesic(1,1); vp = geodesic(1,2); % upp = geodesic(1,3); vpp = geodesic(1,4);
uq = geodesic(end,1); vq = geodesic(end,2); % upq = geodesic(end,3); vpq = geodesic(end,4);

% Initial wrapping point
p = [Radius*cos(up), Radius*sin(up), vp];
% Final wrapping point
...
function [lambda,lsc] = linesearch(f, normf, x, p, sigma1, beta, et, Radius, p0, q0)
% Approximate Norm Descent Line Search
lambda = 1;
i = 1;
lsc = 0;

while norm(f(x+lambda*p,Radius,p0,q0)) > ((1 + et)*normf - sigma1*norm(lambda*p)^2)
   lambda = beta*lambda;
%    lambda = beta^(i);
   i = i+1;
   lsc = lsc + 1;
